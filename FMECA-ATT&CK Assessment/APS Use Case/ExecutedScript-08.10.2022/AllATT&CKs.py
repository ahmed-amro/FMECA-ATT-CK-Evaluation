...
def ImportImpactScore():
	Impacts = []
	with open('CCST.csv', newline='') as f:
...
def ImportRelationships():
	Relationships = []
	with open('FMT.csv', newline='') as f:
		reader = csv.reader(f, delimiter=';', quoting=csv.QUOTE_NONE)
		for row in reader:
...
def ImportFMCT():
	Factors = []
	with open('FMCT.csv', newline='') as f:
		reader = csv.reader(f, delimiter=';', quoting=csv.QUOTE_NONE)
		for row in reader:
			Factors.append(row)
	return Factors				
...
def GetFailureModeFactors(matrix,technique,tactic,FailuteFactors):
	#print("============================================================")
	#print("Matrix ",matrix)
	#print("technique ",technique)
	Factors = ['0','0','0','0','0','0','0']
	for failure in FailuteFactors:
		#print("failure: ",failure)
		if "impact" not in tactic and "network-effects" not in tactic and "remote-service-effects" not in tactic:	
			if  tactic in failure[1] and matrix in failure[0]:
				#print("Found techniques factors")
				#print("failure  ",failure[2:7])
				#print("technique ",technique)
				#print("matrix" ,matrix)
				Factors = failure[2:9]
				return Factors
			...
			if  technique in failure[1] and matrix in failure[0]:
				#print("Found tactics factors")
				#print("failure  ",failure[2:7])
				#print("technique ",technique)
				#print("matrix" ,matrix)
				Factors = failure[2:9]
...
def CalculateImpact(i,comp,OPMODE,ImpactScores,Matrix,FailureMetrics,FailuteFactors):
	"""
	Technique name : i[1]
	Tactics: i[2]
	...
	"""
	FactorsInput = []
	#print("GetFailureModeFactors("+Matrix+","+i[1]+","+i[2]+")")
	FactorsInput = GetFailureModeFactors(Matrix,i[1],i[2],FailuteFactors)
	#print("FactorsInput ",FactorsInput)
	...
	Safety_Factor = float(FactorsInput[1].replace(",","."))
	#print("Safety_Factor: ",Safety_Factor)
	Operational_Factor = float(FactorsInput[0].replace(",","."))
	#print("Operational_Factor: ",Operational_Factor)
	Financial_Factor = float(FactorsInput[3].replace(",","."))
	#print("Financial_Factor: ",Financial_Factor)
	ODC_Factor = float(FactorsInput[4].replace(",","."))
	#print("ODC_Factor: ",ODC_Factor)
	Reputation_Factor = 0#float(FactorsInput[6].replace(",","."))
	Environmental_Factor = 0#float(FactorsInput[5].replace(",","."))
	Privacy_Factor = float(FactorsInput[2].replace(",","."))
	#print("Privacy_Factor: ",Privacy_Factor)
	...
	for Index in ImpactScores:
		if comp[1] == Index[1] and OPMODE == Index[0]: 
			Impact_Sub_Score = Operational_Factor*float(Index[Operational_Index]) + Safety_Factor*float(Index[Safety_Index]) + Privacy_Factor*float(Index[Privacy_Index]) + Financial_Factor*float(Index[Financial_Index])+ ODC_Factor*float(Index[ODC_Index]) + Reputation_Factor*float(Index[Reputation_Index]) + Environmental_Factor*float(Index[Environmental_Index])
		#	print("Impact_Sub_Score: ",Impact_Sub_Score)		
...
def GetTechniqueMitigations(i,TechMitRelationships, Matrix):
	...
	for relationship in TechMitRelationships:
		if i[1] == relationship[2] and Matrix == relationship[1]:
			...
			Effectivness.append(relationship[4])
	Mitigations = list(set(Mitigations))
	Effectivness = list(set(Effectivness))
	#print("Mitigations for "+i[1]+" = "+str(Mitigations))
	return Mitigations,Effectivness

def GetMitigationsCoverage(comp,MitigationCoverage,TechniqueMitigations,Effectivness):
	Coverage = 0
	CoverageList = []
	for each in TechniqueMitigations:
		for mitigation in MitigationCoverage:
			if each == mitigation[0]:
				if mitigation[int(comp[10])] == "1":				
					Coverage = Coverage + 1 #+ Coverage*int(each[4]) # each[4] is effectivness in the FMT
	#print("Coverage for component "+comp[1]+" = "+str(Coverage))
					CoverageList.append(mitigation[0])
	return Coverage,CoverageList
...
def CalculateDetectability(i,comp,Mode,TechMitRelationships, MitigationCoverage,Matrix):
	Detectability = 1
	TechniqueMitigations,Effectivness = GetTechniqueMitigations(i,TechMitRelationships, Matrix)	
	MitigationsCoverage,CoverageList = GetMitigationsCoverage(comp,MitigationCoverage,TechniqueMitigations,Effectivness)
	EffectivnessMitigationList = []
	#print("Calculating Detectability for ",i[1])
	#print("Regarding component ",comp[1])
	#print("Which is covered by ",CoverageList)
	#print("_______________________________________________")
	for mitigation in TechniqueMitigations:
		if mitigation in CoverageList:	
			for measureEffectivness in Effectivness:
				if measureEffectivness != "0" or measureEffectivness != "":
					EffectivnessMitigationList.append(float(measureEffectivness.replace(",",".")))
					#Detectability = Detectability*float(measureEffectivness.replace(",","."))
					#print("======================================================")				
					#print("mitigation : ",mitigation)
					#print("measureEffectivness : ", measureEffectivness)
					#print("======================================================")
				
	#print("MitigationsCoverage :",MitigationsCoverage)
	#print("EffectivnessMitigationList: ",EffectivnessMitigationList)	
	maxNumber = 0
	for number in EffectivnessMitigationList:
		if number > maxNumber:
			maxNumber = number
	if maxNumber != 0:	 		
		Detectability = 1-maxNumber
	else:
		Detectability = 1	
	#print("Detectability of the previous mitigations = ",Detectability)
...
def GetRiskRating(Risk):
	...
	#Max Risk = Maximum Likelihood x Maximum Impact x Maximum Detectability
	#Max Risk = 3,89 	x	5,3  	x	1  = 20,617
	#Min Risk = 0,12	x 	0	x	0 = 0

	#All1
	Low_Threshold = 6.057750979
	Medium_Threshold = 12.11550196
	High_Threshold = 18.17325294
	Critical_Threshold = 24.23100391
...
def MatchwithMobileTechniques(comp):	
	...
	x = 1
	MobileTechnieques = []
	with open('MobileTDT.csv', newline='') as f:
		reader = csv.reader(f, delimiter=';', quoting=csv.QUOTE_NONE)
		for row in reader:			
			temp2 = []
...
def MatchwithICSTechniques(comp):	
	...
	"""
	ICSTechnieques = []
	with open('ICSTDT.csv', newline='') as f:
		reader = csv.reader(f, delimiter=';', quoting=csv.QUOTE_NONE)	
		for row in reader:		
			temp2 = []
			types =  comp[2].split(",")
...
def MatchwithEnterpriseTechniques(comp):	
	...
	temp = []
	with open('EnterpriseTDT.csv', newline='') as f:
		reader = csv.reader(f, delimiter=';', quoting=csv.QUOTE_NONE)
		for row in reader:		
			temp2 = []
			if comp[3] in row[3]:
...
def LoadComponents():	
	...
	"""
	Components = []
	with open('CDT.csv', newline='') as f:
		reader = csv.reader(f, delimiter=';', quoting=csv.QUOTE_NONE)
		for row in reader:
...
for strategy in MitigationStrategies:
	...
	if PrintRisks:
		...
		OutputFileName="RiskAssessmentResults-["+strategy+"].csv"
		OutputFile = open(OutputFileName, 'w')
		Header="Opmode; comp.;tech_ID;MITRE_ID;tech_name;tactic;platforms;CVSS_Exp;Impact;Detectability;Risk;Rating;Mitigations"#;system_requirements"
		...
		for Mode in OPModes:
			...
			if len(CompMobileAttacks)>0:
				...
				for i in CompMobileAttacks:
					...
					Impact = CalculateImpact(i,comp,Mode,ImpactScores,Matrix,FailureMetrics,FailuteFactors)
					...
					if Rating == "Low":
						...
						Critical_Risks += 1			
					ResultString=str(Mode)+";"+comp[1]+";"+"Mob-Tech-"+str(j)+";"+str(i[11])+";"+str(i[1])+";"+str(i[2])+";"+"[]"+";"+str(Likelihood).replace(".", ",")+";"+str(Impact).replace(".", ",")+";"+str(Detectability).replace(".", ",") + ";"+ str(Risk).replace(".", ",")+";"+str(Rating)+";"+str(MitigationsList)+";"
					j = j +1 
					if PrintRisks:
			...
			if len(CompEnterpriseAttacks)>0:
				...
				for i in CompEnterpriseAttacks:
					...
					if Rating == "Low":
						...
						Critical_Risks += 1				
					ResultString = str(Mode)+";"+comp[1]+";"+"IT-Tech-"+str(j)+";"+str(i[11])+";"+str(i[1])+";"+str(i[2])+";"+str(i[3])+";"+str(Likelihood).replace(".", ",")+";"+str(Impact).replace(".", ",")+";"+str(Detectability).replace(".", ",") + ";"+ str(Risk).replace(".", ",")+";"+str(Rating)+";"+str(MitigationsList)+";"#+str(i[12])
					if PrintRisks:
						OutputFile.write(ResultString+"\n")  # python will convert \n to os.linesep
					j = j +1 
					OverallRisk = OverallRisk + Risk
			...
			if len(CompICSAttacks)>0:
				...
				for i in CompICSAttacks:
					...
					if Rating == "Low":
						...
						Critical_Risks += 1				
					ResultString = str(Mode)+";"+comp[1]+";"+"ICS-Tech-"+str(j)+";"+str(i[11])+";"+str(i[1])+";"+str(i[2])+";"+str(i[3])+";"+str(Likelihood).replace(".", ",")+";"+str(Impact).replace(".", ",")+";"+str(Detectability).replace(".", ",") + ";"+ str(Risk).replace(".", ",")+";"+str(Rating)+";"+str(MitigationsList)+";"
					if PrintRisks:
						OutputFile.write(ResultString+"\n")  # python will convert \n to os.linesep
					j = j +1 
					OverallRisk = OverallRisk + Risk	
